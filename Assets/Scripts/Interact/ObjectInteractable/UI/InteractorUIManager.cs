...
namespace InteractSystem
{
    ...
    public class InteractorUIManager
    {
        ...
        public GameObject ShowUI(InteractTipSO config, Action action = null)
        {
            ...
            if (config = tipUIConfig)
            {
                Debug.Log("相同的Config，无需更新");
                return activeObj;
            }
            else
            {
        ...
        public void HideUI(InteractTipSO config,Action action = null)
        {
            ...
            {
                UIView view;
                //注意如果一直有物体不通过这个脚本进行创建，但是一直通过这个脚本进行隐藏，就会导致一直GetComponent（会在创建时缓存UIView接口对象）
                if (uiDic.TryGetValue(obj, out view))
                {
                ...
                //如果物体身上没有UIView接口就会直接暴力触发委托并隐藏UI
                else
                {
                    action?.Invoke();
                    obj.SetActive(false);
